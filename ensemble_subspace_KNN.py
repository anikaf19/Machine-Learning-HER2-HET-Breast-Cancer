...
# "impute" Nan values (which KNN classifier cannot handle)
...
ensemble_knn = BaggingClassifier(
    estimator=KNeighborsClassifier(n_neighbors=8,leaf_size=5, weights='distance'),
    n_estimators=20,
    max_samples=0.8,
    bootstrap=True,
...
y_train_pred = ensemble_knn.predict(X_train)
y_pred = ensemble_knn.predict(X=X_test)
...
y_train_proba = ensemble_knn.predict_proba(X_train)[:,1] # predict probabilities for AUC-ROC
...
y_test_proba = ensemble_knn.predict_proba(X_test)[:,1] # predict probabilities for AUC-ROC
