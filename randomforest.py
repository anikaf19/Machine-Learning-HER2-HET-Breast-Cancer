...
# transcripts = pd.read_csv("/Users/anikaflorin/Documents/Thesis/data/filtered_merged_output.csv")
...
# load data subsets
# het = pd.read_csv('/Users/Documents/Thesis/data/het_subset.csv')
...
y_het_pred = rf.predict(X=X_test)
# Compute performance metrics
train_accuracy = accuracy_score(y_train,y_train_pred)
train_precision = precision_score(y_train,y_train_pred,pos_label=1)
train_confusion = confusion_matrix(y_train,y_train_pred)
    
y_train_proba = rf.predict_proba(X_train)[:,1] # predict probabilities for AUC-ROC
...
y_test_proba = rf.predict_proba(X_test)[:,1] # predict probabilities for AUC-ROC
...
correlations = transcripts.drop(columns=['Sample_ID', 'Patient_ID']).corr(numeric_only=True)['pCR'].drop('pCR')
top_features = correlations.abs().sort_values(ascending=False).head(100)
...
plt.title('Top Features Correlated with pCR')
